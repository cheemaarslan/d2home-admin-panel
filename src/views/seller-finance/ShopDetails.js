...
const ShopDetails = () => {
  ...
  const financialTableRows = [
    { desc: 'Total Sale Amount', sub: '', total: shopData.total_sales, isBold: false },
    { desc: 'D2Home Commission:', sub: shopData.total_commission, total: '', isBold: false },
    { desc: 'D2Home Discounts (site promotions):', sub: shopData.sum_of_order_discounts, total: '', isBold: false },
    { desc: 'Total D2Home Commission:', sub: '', total: shopData.total_charges_and_discounts, isBold: true },
    { desc: 'Subtotal:', sub: '', total: shopData.net_amount_payable, isBold: true },
    { desc: 'The amount to be transferred:', sub: '', total: shopData.net_amount_payable, isBold: true, isFinal: true },
  ];

  return (
    <div style={{ padding: '20px', backgroundColor: '#f0f2f5', fontFamily: FONT_FAMILY }}>
      <div style={{ maxWidth: 840, margin: '0 auto', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', backgroundColor: '#fff', borderRadius: 8 }}>
        <div style={{ textAlign: 'right', padding: '25px 10px 20px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 24, marginBottom: 25, padding: '0 25px' }}>
          <div style={{ flex: '1 1 300px' }}>
            <div style={{ marginBottom: 16 }}>
              <Text strong style={{ fontSize: 14 }}>Invoice No:</Text>
              <Text style={{ fontSize: 14, marginLeft: 8 }}>#100{derivedInvoiceMeta.invoiceNumber}</Text>
            </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, fontSize: 13 }}>
            <thead>
              <tr style={{ backgroundColor: '#e49000' }}>
                <th style={{ padding: '10px 12px', textAlign: 'left', color: '#333', fontWeight: 600, borderBottom: '3px solid black' }}>
                  Description
                </th>
                <th style={{ padding: '10px 12px', textAlign: 'right', color: '#333', fontWeight: 600, borderBottom: '3px solid black' }}>
                  Sub Total
                </th>
                <th style={{ padding: '10px 12px', textAlign: 'right', color: '#333', fontWeight: 600, borderBottom: '3px solid black' }}>
                  Total
            ...
            <tbody>
              {financialTableRows.map((row, index) => (
                <tr key={index}>
                  <td style={{ padding: '10px 12px', borderBottom: '1px solid black', color: row.isBold ? '#333' : '#555', fontWeight: row.isBold ? 600 : 400 }}>
                    {row.desc}
                  </td>
                  <td style={{ padding: '10px 12px', textAlign: 'right', color: '#555', borderBottom: '1px solid black' }}>
                    {row.sub !== '' ? `$${Number(row.sub).toFixed(2)}` : ''}
                  </td>
                  <td style={{ padding: '10px 12px', textAlign: 'right', color: row.isBold ? '#333' : '#555', fontWeight: row.isBold ? 600 : 400, fontSize: row.isFinal ? '15px' : '13px', borderBottom: '1px solid black' }}>
        ...
        <div style={{ padding: '0 25px', marginBottom: 25 }}>
          <Title level={5} style={{ marginBottom: 12, color: '#333', fontWeight: 600 }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, fontSize: 13 }}>
              ...
              <tbody>
                {normalizedOrders.map((order, idx) => (
                  <tr key={order.id}>
                    <td style={{ padding: '10px 12px', borderBottom: '1px solid black', color: '#555' }}>
                      {idx + 1}
                    </td>
                    <td style={{ padding: '10px 12px', borderBottom: '1px solid black', color: '#555' }}>
                      {order.id}
                    ...
                    <td style={{ padding: '10px 12px', textAlign: 'right', borderBottom: '1px solid black', color: '#333', fontWeight: 600 }}>
                      ${Number(order.total_price).toFixed(2)}
                    </td>
                  </tr>
                ))}
                <tr>
                  <td colSpan={3} style={{ padding: '10px 12px', textAlign: 'right', color: '#333', fontWeight: 600, borderBottom: '1px solid black' }}>
                    Total:
                  </td>
                  <td style={{ padding: '10px 12px', textAlign: 'right', color: '#333', fontWeight: 600, fontSize: '14px', borderBottom: '1px solid black' }}>
        ...
        <Paragraph style={{ marginBottom: 25, fontSize: '12px', color: '#777', padding: '0 25px', lineHeight: 1.5 }}>
          This credit will be credited to your account in the next few days BAN: Michael transferred. If you have any questions about your receipt, please contact our service center [{companyDetails.email}]. Details of this invoice can be found on the attached page.
        </Paragraph>

        <div style={{ textAlign: 'center', marginTop: 30, padding: '20px 25px', borderTop: '1px solid #e8e8e8' }}>
