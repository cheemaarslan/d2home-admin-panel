...
export default function OrderCart() {
 ...
 function productCalculate(newDiscountAmount) {
  const products = formatProducts(filteredCartItems);
  const discount = (newDiscountAmount ?? parseFloat(discountAmount)) || 0;

  setLoading(true);
  orderService.calculate(products).then(({ data }) => {
    ...
    const orderData = {
      ...
      order_total:
        ...
        (product?.total_shop_tax || 0) +
        (product?.delivery_fee || 0) +
        (product?.service_fee || 0) -
        (product?.coupon_price || 0) -
        discount,
    };
  ...
  const handleApplyDiscount = (amount) => {
    ...
    if (!isNaN(parsedAmount) && parsedAmount >= 0) {
      setDiscountAmount(parsedAmount); // Update state
      // Optionally dispatch to Redux if needed
      // dispatch(updateDiscount(parsedAmount));
    } else {
      console.error('Invalid discount amount');
      setDiscountAmount(0); // Reset to 0 if invalid
    }
  };
    ...
    <Card>
      ...
      <div className='card-save'>
        ...
        <Row className='all-price-row'>
          <Col span={24} className='col' style={{ display: 'flex', justifyContent: 'flex-end' }}>
            {!showDiscountInput ? (
              <Button
                type="primary"
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <Button
                  type="primary"
                  onClick={() => {
                    handleApplyDiscount(discountAmount);
                    productCalculate();
