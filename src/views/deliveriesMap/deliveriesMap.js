...
// Custom Marker Component with Online/Offline Visual Indicator
const DeliverymanMarker = ({ url, isOnline, onClick }) => {
  return (
    <div
      style={{
        position: 'relative',
    ...
    >
      <Avatar
        ...
        size={50}
        style={{
          border: isOnline ? '4px solid #52c41a' : '4px solid #f5222d', // Green for online, Red for offline
...
export default function DeliveriesMap() {
  ...
  return (
    <Card
      title={t('deliveries latest code')}
      className='delivery'
      extra={
        <Select
          ...
          <Col span={18}>
            ...
            >
              ...
              <MapCustomMarker center={center} handleLoadMap={handleLoadMap}>
                {delivery.map((item) => {
                  const isOnline = item.delivery_man_setting?.online === true;
                  const avatarUrl = IMG_URL + item.img;
