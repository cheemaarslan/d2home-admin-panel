...
export default function UserAddRole() {
  ...
  const onFinish = (values) => {
    ...
    const nextUrl = locations.pathname.includes('/user/delivery/')
      ? 'deliveries/list'
      : 'users/admin';
    ...
    userService.create(body).then(({ data }) => {
      if (role === 'deliveryman') {
        const deliveryParams = {
          ...
          police_check: values.police_check,
          working_right: values.working_right,

          // PHOTOS from state
          license_front_photo: values.license_front_photo,
          license_back_photo: values.license_back_photo,
          passport_front_photo: values.passport_front_photo,
          passport_back_photo: values.passport_back_photo,
          visa_copy_front_photo: values.visa_copy_front_photo,
          visa_copy_back_photo: values.visa_copy_back_photo,
          police_check_photo: values.police_check_photo,
          working_right_photo: values.working_right_photo,
        };

        deliveryService.create(deliveryParams).then(() => {
        ...
        const userParamsData = { ...user.params, role: data.role };
      ...
      if (err?.response?.data?.params) {
        Object.values(err.response.data.params).forEach((errorArray) =>
          ...
          {role === 'deliveryman' && (
            <>
              ...
              <DelivertSetting
                ...
                visaFront={visaFront}
                setVisaFront={setVisaFront}
                visaBack={visaBack}
                setVisaBack={setVisaBack}
                policePhoto={policePhoto}
                setPolicePhoto={setPolicePhoto}
                workingPhoto={workingPhoto}
                setWorkingPhoto={setWorkingPhoto}
              />
            </>
          )}
          <Col span={24}>
